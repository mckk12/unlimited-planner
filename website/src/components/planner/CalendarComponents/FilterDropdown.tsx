...
const FilterDropdown: FC<FilterDropdownProps> = memo(({
    isOpen,
    onToggleOpen,
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                ...
                className="cursor-pointer flex items-center gap-2 rounded-lg border border-slate-600 bg-slate-700 px-4 py-2 text-sm font-semibold text-slate-100 hover:bg-slate-600 transition-colors"
            >
                Filter
            ...
            {isOpen && (
                <div className="absolute right-0 top-full mt-2 w-64 rounded-lg border border-slate-600 bg-slate-800 p-4 shadow-lg z-10">
                                            ...
                                            {group.items?.map(item => (
                                                <label key={item.id} className="flex items-center gap-2 cursor-pointer">
                                                    <input
                                                        ...
                                                        className="rounded border-slate-600 bg-slate-700 accent-cyan-600"
                                                    />
                    ...
                    {selectedAttributes.size > 0 && (
                        ...
                        >
                            Clear filters ({selectedAttributes.size})
                        </button>
                    )}
                </div>
            )}
